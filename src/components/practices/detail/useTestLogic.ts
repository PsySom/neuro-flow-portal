...
**Атрибуция:** Lovibond & Lovibond, UNSW; www.psy.unsw.edu.au/dass`;
  ...
  const calculateK10Result = (score: number): TestResult => {
    let interpretation = '';
    let recommendations = '';

    // K10 interpretation based on ABS categories (score range 10-50)
    if (score >= 10 && score <= 15) {
      interpretation = 'Низкий дистресс';
      recommendations = `**Результат K10: ${score}/50 - Низкий дистресс**

🌟 Ваш уровень психологического дистресса низкий.

**Рекомендации для профилактики:**
✓ Поддерживайте здоровый режим сна
✓ Регулярная физическая активность
✓ Щадящий план самоподдержки
✓ Стабильный распорядок дня
✓ Повторная проверка через 2-4 недели

**Атрибуция:** K10 © Ronald C. Kessler и соавт.; free to use. Источник: Harvard NCS.`;
    } else if (score >= 16 && score <= 21) {
      interpretation = 'Умеренный дистресс';
      recommendations = `**Результат K10: ${score}/50 - Умеренный дистресс**

⚠️ Умеренный уровень психологического дистресса.

**Рекомендации:**
• Психообразование о природе дистресса
• Дыхательные упражнения и техники релаксации
• Ограничение перегрузки новостями и экранным временем
• Ведение трекинга симптомов
• Обратитесь за консультацией, если дистресс мешает повседневным делам

**Атрибуция:** K10 © Ronald C. Kessler и соавт.; free to use. Источник: Harvard NCS.`;
    } else if (score >= 22 && score <= 29) {
      interpretation = 'Высокий дистресс';
      recommendations = `**Результат K10: ${score}/50 - Высокий дистресс**

🔶 Высокий уровень психологического дистресса.

**Рекомендации:**
🏥 **Рекомендуется консультация специалиста** (психолог/врач)
• Совместный план: режим сна, физическая нагрузка
• Поведенческие техники управления стрессом
• Социальная поддержка и активизация ресурсов
• Мониторинг состояния в приложении

**Атрибуция:** K10 © Ronald C. Kessler и соавт.; free to use. Источник: Harvard NCS.`;
    } else if (score >= 30 && score <= 50) {
      interpretation = 'Очень высокий дистресс';
      recommendations = `**Результат K10: ${score}/50 - Очень высокий дистресс**

🚨 **Очень высокий уровень психологического дистресса**

**Настоятельно рекомендуется:**
🏥 **Обязательная очная/онлайн оценка у специалиста**
• Приоритетная запись к психологу или врачу
• Комплексный подход к лечению
• Профессиональное сопровождение

📞 **Службы помощи:**
• Телефон доверия: 8-800-2000-122 (бесплатно, круглосуточно)
• При критическом состоянии: 112

**Атрибуция:** K10 © Ronald C. Kessler и соавт.; free to use. Источник: Harvard NCS.`;
    } else {
      interpretation = 'Некорректный балл';
      recommendations = 'Ошибка в подсчете баллов. Пожалуйста, пройдите тест заново.';
    }
  ...
  const calculateK6Result = (score: number): TestResult => {
    let interpretation = '';
    let recommendations = '';

    // K6 interpretation based on established thresholds (score range 0-24)
    if (score >= 0 && score <= 4) {
      interpretation = 'Минимальный дистресс';
      recommendations = `**Результат K6: ${score}/24 - Минимальный дистресс**

🌟 Ваш уровень психологического дистресса минимален.

**Рекомендации:**
✓ Профилактические меры для поддержания благополучия
✓ Поддерживайте здоровый образ жизни
✓ Регулярная самопроверка для раннего выявления изменений
✓ Повторная проверка через 2-4 недели

**Атрибуция:** K6 © Ronald C. Kessler и соавт.; free to use. Источник: Harvard NCS.`;
    } else if (score >= 5 && score <= 12) {
      interpretation = 'Умеренный дистресс';
      recommendations = `**Результат K6: ${score}/24 - Умеренный дистресс**

⚠️ Умеренный уровень психологического дистресса.

**Рекомендации:**
• Психообразование о стрессе и его влиянии
• Техники самопомощи и управления стрессом
• Опция беседы со специалистом при значительных помехах в повседневности
• Повторная оценка через 2-4 недели для мониторинга

**Атрибуция:** K6 © Ronald C. Kessler и соавт.; free to use. Источник: Harvard NCS.`;
    } else if (score >= 13 && score <= 24) {
      interpretation = 'Выраженный дистресс';
      recommendations = `**Результат K6: ${score}/24 - Выраженный психологический дистресс**

🚨 **Вероятен выраженный дистресс/риск серьёзного психического расстройства**

**Настоятельно рекомендуется:**
🏥 **Консультация специалиста** (психолог/врач)
• При быстром ухудшении состояния — приоритетная запись
• Профессиональная оценка и план лечения
• Регулярное наблюдение

📞 **Локальные службы помощи:**
• Телефон доверия: 8-800-2000-122 (бесплатно, круглосуточно)
• Экстренные службы: 112
...
**Атрибуция:** K6 © Ronald C. Kessler и соавт.; free to use. Источник: Harvard NCS.`;
  ...
  const calculateTestResult = (item: any) => {
    ...
    if (item.title.includes("Шкала депрессии Бека")) {
      ...
      result = calculateK10Result(score);
    } else if (item.keys === "K6") {
      Object.values(answers).forEach(answer => {
        const answerIndex = parseInt(answer.charAt(0));
        if (!isNaN(answerIndex)) {
          score += answerIndex;
      ...
      result = calculateK6Result(score);
    ...
    if (item.keys === "PHQ-4" || item.keys === "PHQ-9" || item.keys === "GAD-7" || item.keys === "DASS-21" || item.keys === "K10" || item.keys === "K6") {
      // For PHQ, GAD, DASS and Kessler tests, use the detailed recommendations from the function
